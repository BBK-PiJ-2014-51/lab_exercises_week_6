	...
	public static void main(String[] args) {
		...
		while (true){
			...
			try {
				...
				xCoord = Integer.parseInt(inputString);
				System.out.println("Enter a coordinate Y: ");
				inputString = br.readLine();
				yCoord = Integer.parseInt(inputString);
				System.out.println("Enter a coordinate Z: ");
				inputString = br.readLine();
				zCoord = Integer.parseInt(inputString);
			} catch (NumberFormatException | IOException e) {
				e.printStackTrace();
			}
			Result result = target.fire(xCoord, yCoord, zCoord);
			result.announce();
			if (result.equals(Result.HIT)) {
				...
				if (inputString.toUpperCase().charAt(0) != 'Y')break;
				else target = new Target(10);
			}
		}
	}
}
